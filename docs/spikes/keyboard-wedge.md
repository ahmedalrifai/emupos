...
9. Notepad as administrator (`kb-11`, run twice, Arabic still the foreground layout): SendInput accepted **28 of 28** both times. Recheck (`re-03-admin-notepad.txt`, run twice from a PowerShell confirmed not elevated, US layout): accepted 28 of 28 both times, and **nothing appeared** in the elevated Notepad. Windows discarded the keystrokes and still reported every one as accepted, as Microsoft documents for UIPI.
