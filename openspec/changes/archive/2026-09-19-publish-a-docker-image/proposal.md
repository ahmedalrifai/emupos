...
- **The bundled configuration ships only devices whose container wiring is the real wiring**: a printer on `tcp: { host: 0.0.0.0, port: 9100 }`, which is how a network receipt printer attaches, and a keyboard scanner with `typed_by: client`, which types real keystrokes on the operator's machine. `api: { host: 0.0.0.0 }` so published ports reach it.
- **No serial device is bundled, because the container alone cannot create one.** A pseudo-terminal created inside a container belongs to that container's own devpts instance, so the link a POS on the host would open points at a device that does not exist there — demonstrated on macOS, where the container is inside Docker Desktop's Linux VM. What a container cannot do alone, a process next to the POS can: a relay running where the POS is creates the port and carries the bytes, which is the `bridge-serial-ports-to-the-client` change. Until that lands, the documentation points at running emupos on the host for serial devices.
- **A TCP stand-in for a serial device is still refused.** Not because it tests nothing — a POS behind a relay does open a real tty and does run `tcsetattr` — but because emupos observes the line settings only on a pseudo-terminal it holds (`connections.py:96` spawns `watch_framing` from `_open_pty`; `_open_tcp` never observes). A scale on a bare TCP port therefore answers a POS configured at 19200 8N1 with a clean weight and no warning at all, which is the one diagnostic emupos has for the commonest serial mistake.
...
- **Documentation:**
  - `docs/docker.md`: the `docker run` line, mounting your own configuration and a receipts volume, why the bundled configuration binds `0.0.0.0` and why publishing to `127.0.0.1:PORT:PORT` keeps it private, that serial devices need emupos on the host and why a container cannot present one, and that keyboard scans are typed by the machine running `emupos scan`.
