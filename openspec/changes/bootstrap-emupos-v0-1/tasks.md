...
- [ ] 13.4 `docs/configuration.md`: human-readable reference for every `emupos.yaml` key and the device profile format, with a complete example per device type and a pointer to `emupos config schema` for editor autocomplete.
...
- [ ] 13.8 `SECURITY.md`: supported versions, private vulnerability reporting through GitHub security advisories, and the local API's threat model (localhost only, browser-request protection, no authentication).
- [ ] 13.9 `CODE_OF_CONDUCT.md` (Contributor Covenant) with the maintainer contact for reports.
- [ ] 13.10 `.github/ISSUE_TEMPLATE/`: bug report (asks for `emupos doctor --json` output, OS, config and a hex capture if relevant), device or protocol request (model, manual link, transport), and profile correction (e.g. code-page numbers, with a self-test page photo); plus `config.yml` pointing security reports to `SECURITY.md`.
- [ ] 13.11 `.github/pull_request_template.md`: Conventional Commit title reminder, linked issue, checklist for tests and fixtures added, manual section cited for new commands, and docs updated.

## 14. Release pipeline

- [ ] 14.1 release-please configuration for a Python package on the `main` branch, generating `CHANGELOG.md` with a Breaking section and bumping the `pyproject.toml` version.
