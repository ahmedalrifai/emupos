...
- **THEN** a `printer.codepage.unsupported` event is emitted naming code page number 37 and PC864
- **AND** the receipt image shows `TOTAL` followed by one placeholder occupying one 12 × 24 dot cell
...
- **WHEN** the POS sends `1b 74 16`
- **THEN** a `printer.codepage.unsupported` event is emitted naming code page number 22 and PC864
