...
The documentation SHALL describe installing emupos with `uv tool install emupos` as the recommended method, and also with `uvx emupos`, `pipx install emupos` and `pip install emupos`, and SHALL describe running the published container image as an alternative that needs no Python on the machine. `README.md` SHALL show the recommended method and link the installation guide that carries the others. The documentation SHALL include per-operating-system setup guides for the external prerequisites: com0com for serial devices on Windows, Accessibility permission for keyboard-wedge scanning on macOS, and X11 with libXtst for keyboard-wedge scanning on Linux. It SHALL include a Docker guide stating which ports to publish, that publishing to `127.0.0.1` keeps the control API private, that pseudo-terminal serial ports do not cross the container boundary, and that keyboard-mode scans are typed by the machine that runs `emupos scan`.
